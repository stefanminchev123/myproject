	...
	for ($i = $start_image; $i < count($this->product->images); $i++) {
		...
		<div class="floatleft">
			...
			if(VmConfig::get('add_img_main', 1)) {
				$prod_image = $image->displayMediaThumb('class="product-image" style="cursor: pointer"',false,$image->file_description);
				if ( strpos($prod_image, 'spec2') == false && strpos($prod_image, 'spec3') == false) {
					echo $prod_image;
				}
